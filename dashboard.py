...
st.sidebar.info(
    "Données simulées pour portfolio tech/business."
)
...
if page == "Accueil":
    ...
    st.markdown(
        "## Beauty & Cosmetics Business Intelligence"
    )
...
""")
    ...
    col4.metric(
        "Avg Order Value",
        f"{average_order_value:,.0f} FG",
        "+5%"
    )

    st.markdown("## Sales Over Time")

    sales_over_time = orders.groupby(
        "date"
    )["revenue"].sum()
...
""")
    ...
    sales_by_channel = filtered_orders.groupby(
        "channel"
    )["revenue"].sum().sort_values(
        ascending=False
    )
    ...
    if sales_by_channel.empty:
        ...
        fig2.patch.set_facecolor("#F8F6F2")
...
# =====================================================
    ...
    st.info(
        "Résumé exécutif des performances commerciales et recommandations stratégiques."
    )

    best_channel = orders.groupby(
        "channel"
    )["revenue"].sum().idxmax()

    best_category = orders.groupby(
        "category"
    )["revenue"].sum().idxmax()

    best_product = orders.groupby(
        "product"
    )["quantity"].sum().idxmax()
    ...
    st.success(
        f"Top revenue category: {best_category}"
    )
